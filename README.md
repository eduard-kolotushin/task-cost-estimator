...
Тело страницы — JSON **TipTap/ProseMirror** (`type: doc`, таблица с `table` / `tableRow` / `tableCell`), строка в поле `wiki_page_body`. Сборка вручную в [`src/wiki/adf.py`](src/wiki/adf.py) по образцу [`example_wiki_page_est_table.txt`](example_wiki_page_est_table.txt): пять колонок (**Команда**, **Компонент**, **Этап**, **Оценка**, **Декомпозиция**), секции по «Команде», колонка **Декомпозиция** — нумерованный список (`orderedList`). Формат **ADF (Atlassian)** в UI TaskTracker не используется. Ошибки **SSL/TLS** при работе агента обычно от **TaskTracker** (`TASKTRACKER_VERIFY_SSL`) или **LLM Hub** (`HUB_VERIFY_SSL`) / **GigaChat** (`GIGACHAT_VERIFY_SSL`).
