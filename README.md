...
Тело страницы — JSON ProseMirror (`wiki_page_body`). Сборщик в [`src/wiki/prose.py`](src/wiki/prose.py) повторяет **атрибуты узлов** из реального примера [`example_wiki_page_est_table.txt`](example_wiki_page_est_table.txt): `table` (id, tableId, columnWidths, filters, …), `tableRow` (originalRowIndex, …), `tableCell` (numberedColumn, …), жирные заголовки через `marks`. В примере из файла таблица на **3** колонки (Этап / Оценка / Комментарий); агент по ТЗ заполняет **5** колонок (добавлены Команда и Компонент), но схема TipTap совместима с порталом.
