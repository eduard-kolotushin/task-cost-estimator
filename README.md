...
Тело страницы — **Atlassian Document Format (ADF)** в поле `wiki_page_body` (JSON-строка). Сборка — пакет [**atlassian-doc-builder**](https://pypi.org/project/atlassian-doc-builder/) в [`src/wiki/adf.py`](src/wiki/adf.py): **структура таблицы** как в [`example_wiki_page_est_table.txt`](example_wiki_page_est_table.txt) — заголовок колонок, затем **блоки по «Команде»** (строка с одной ячейкой на всю ширину), под ними строки данных, в конце **Итого**; колонка **Декомпозиция** — `orderedList`. Пример в файле — старый ответ API (TipTap); актуальный формат — ADF, см. [структуру ADF](https://developer.atlassian.com/cloud/jira/platform/apis/document/structure/). При первом построении узла ADF пакет **atlassian-doc-builder** один раз подгружает JSON-схему с **unpkg.com** (`urllib`); при ошибке проверки сертификата в [`src/wiki/adf_schema_patch.py`](src/wiki/adf_schema_patch.py) выполняется повтор без проверки TLS. Остальные **SSL/TLS**-ошибки обычно от **TaskTracker** (`TASKTRACKER_VERIFY_SSL`) или **LLM Hub** (`HUB_VERIFY_SSL`) / **GigaChat** (`GIGACHAT_VERIFY_SSL`).
