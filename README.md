...
Тело страницы — JSON **TipTap/ProseMirror** (`type: doc`, таблица с `table` / `tableRow` / `tableCell`), строка в поле `wiki_page_body`. Сборка в [`src/wiki/adf.py`](src/wiki/adf.py) по образцу [`QA.rtf`](QA.rtf) / [`example_wiki_page_est_table.txt`](example_wiki_page_est_table.txt): **три колонки** на странице (**Этап**, **Оценка**, **Декомпозиция**), секции по команде; в API строки по-прежнему с `komanda` и `komponent` (VIEW). Колонка **Декомпозиция** — нумерованный список (`orderedList`). Ошибки **SSL/TLS** при работе агента обычно от **TaskTracker** (`TASKTRACKER_VERIFY_SSL`) или **LLM Hub** (`HUB_VERIFY_SSL`) / **GigaChat** (`GIGACHAT_VERIFY_SSL`).
