...
Тело страницы — JSON **TipTap/ProseMirror** (`type: doc`, таблица с `table` / `tableRow` / `tableCell`), строка в поле `wiki_page_body`. Сборка в [`src/wiki/adf.py`](src/wiki/adf.py) по [`QA.rtf`](QA.rtf): **три колонки**, **четыре секции** и **фиксированный набор этапов** в каждой (пустые ячейки — 0 и без декомпозиции). В API в `rows` передаются только заполненные этапы; `komanda` / `komponent` (VIEW). **Декомпозиция** — `orderedList`, если текст не пустой. Ошибки **SSL/TLS** — от **TaskTracker** (`TASKTRACKER_VERIFY_SSL`) или **LLM Hub** / **GigaChat**.
